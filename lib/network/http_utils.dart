...
class HttpUtils {
  ...
  Future<List<ArticleModel>> requestTopArticle() async{
    var res  = await dio.get(NetConstant.TOP_ARTICLE,options: _options.merge(extra: {"noCache": false}));
  ...
  Future<MainArticle> requestMainArticle(String page) async{
    ...
    MainArticle ret = BaseResponse<dynamic>.fromJson(res.data).data.map((f)=>MainArticle.fromJson(f)).toList();
    return ret;
  }
}
